...
from napr.utils import label_encode, split_train_test
...
def test_split_train_test():
    ...
    X_train, X_test = split_train_test(  # type: ignore
        data=pd.DataFrame(np.random.rand(10, 3), columns=np.array(["a", "b", "c"])),
        test_size=0.4,
    )
    ...
    X_train_1, X_test_1 = split_train_test(  # type: ignore
        data=data, test_size=test_size, random_state=random_state_1
    )
    X_train_2, X_test_2 = split_train_test(  # type: ignore
        data=data, test_size=test_size, random_state=random_state_2
    )
...
def test_label_encode(
    ...
    with exception:
        train_encoded, test_encoded, labels = label_encode(
            train,
            test,
        )
