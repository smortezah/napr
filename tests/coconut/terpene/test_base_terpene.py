...
from napr.coconut.terpene.explore import Plot
...
def test_base_class(data):
    ...
    assert isinstance(terpene.plot, Plot)
...
def test_preprocess(data, dropped_columns):
    ...
    terpene = Terpene(data=data)
    terpene.preprocess(random_state=777, unknown_value=9999, dropped_columns=dropped_columns)

    assert terpene.data.shape == (
        data.shape[0],
        data.shape[1] + len_expanded_bcutDescriptor + len_expanded_textTaxa - len(dropped_columns),
    )
...
def test_dim_reduce(data, dropped_columns):
    ...
    len_target_col = 1
    n_components = 2
    data_reduced = terpene.dim_reduce(model=PCA(n_components=n_components), inplace=False)
