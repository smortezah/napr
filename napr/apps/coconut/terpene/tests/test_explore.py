...
def data():
    ...
    num_rows = len(chemicalSubClass_21_3)
    return pd.DataFrame({
        'molecular_weight': [random.random() for _ in range(num_rows)],
        'alogp': [random.random() for _ in range(num_rows)],
        'npl_score': [random.random() for _ in range(num_rows)],
        'lipinskiRuleOf5Failures': [random.random() for _ in range(num_rows)],
        'hBondAcceptorCount': [random.random() for _ in range(num_rows)],
        'hBondDonorCount': [random.random() for _ in range(num_rows)],
