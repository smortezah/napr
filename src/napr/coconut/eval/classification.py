...
def _scores_row(
    ...
    if "accuracy" in scoring:
        accuracy = metrics.accuracy_score(y_test, pred, sample_weight=sample_weight)
        row["accuracy"] = accuracy
    if "precision" in scoring:
        precision = metrics.precision_score(
            y_test, pred, sample_weight=sample_weight, average=average
        )
        row["precision"] = precision
    if "recall" in scoring:
        recall = metrics.recall_score(y_test, pred, sample_weight=sample_weight, average=average)
        row["recall"] = recall
    if "f1" in scoring:
        f1 = metrics.f1_score(y_test, pred, sample_weight=sample_weight, average=average)
        row["f1"] = f1
    if "conf_mat" in scoring:
        conf_mat = metrics.confusion_matrix(
            y_test, pred, sample_weight=sample_weight, labels=labels, normalize=None
        )
...
def eval_classification(
    ...
    Args:
        estimators: The classifiers. It can be either of a single scikit-learn classification model,
            a list of models or a dictionary of classifiers with their names (string) as keys.
        X: Input data.
        y: The target data (1d).
        test_data: The test/validation data, in the form of (X_test, y_test). If not provided,
            the "test_size" will be considered to split the train-test. Defaults to None.
        test_size: Fraction of the test/evaluation data. It defaults to 0.2 in case neither of
            "test_data" nor "test_size" is provided. Defaults to None.
    ...
    if isinstance(estimators, list):
        ...
        estimators: dict = {name: est for name, est in zip(estimator_names, estimators)}
