...
class Preprocess:
    ...
    def preprocess(self, **kwargs) -> pd.DataFrame:
        ...
        self.data.drop(self.dropped_columns + self.target_columns, axis=1, inplace=True)
        self.data = pd.concat([self.data, target_data], axis=1)
        return self.data

    def _split_bcutDescriptor(self) -> None:
        """Split the bcutDescriptor column into multiple columns and concatenate to the data."""
        if "bcutDescriptor" not in self.data.columns:
            return None

        splitted = self.data["bcutDescriptor"].apply(lambda x: x[1:-1]).str.split(",", expand=True)
    ...
    def _extract_tax(self) -> None:
        """Extract the taxonomy from the textTaxa column and concatenate to the data."""
    ...
    def dim_reduce(self, **kwargs) -> pd.DataFrame:
        ...
        if "model" in kwargs:
            if isinstance(kwargs["model"], str):
                raise ValueError("model object must have implemented fit_transform method.")
            self.model = kwargs["model"]

        columns = [col for col in self.data.columns if col not in self.target_columns]
        data_reduced = self.model.fit_transform(self.data[columns])
        df_reduced = pd.DataFrame(
            data=data_reduced,
            index=self.data.index,
            columns=["d" + str(i) for i in range(data_reduced.shape[1])],  # type: ignore
        )
