...
from napr.coconut.plotting._base import label_subplot
...
class Plot:
    ...
    def dist_subclass_mw_logp_nplscore(
        ...
        """Plot the distribution of subclasses, molecular weight, logP, and NPL-score.
        ...
        def _get_pies_data(data: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
            ...
            data_largepie = pd.concat(
                [
                    data_pie,
                    pd.Series({"other": data_smallpie.sum()}),
                ]
            )
        ...
        ax[0].pie(
            ...
            startangle=-1.0 / 360 * data_largepie.iloc[0],
            autopct="%.0f%%",
        ...
        def _plot_table(data, bbox, ax):
            table = ax.table(cellText=[(i, f"{v:.1f}") for i, v in data.items()], bbox=bbox)
            # Format
        ...
        def _plot_hist_table(data, ax, xlim=None, xlabel=None, ylabel=None, bbox=None):
    ...
    def violin_mw_logp_nplscore(
        ...
        """Plot the distribution of molecular weight, logP and NPL-score for each terpene subclass.
        ...
        def _plot_violin(
            ...
            sns.violinplot(
                ...
                density_norm="width",
                inner=None,
                palette=sns.color_palette("Dark2", n_colors=len(order)),
                linewidth=1.2,
            )
            # Mean
            ...
            # Axes
            ax.yaxis.set_ticks(range(len(yticklabels)))
    ...
    def lipinsky(self, figsize: tuple[float, float] = (6, 8)) -> tuple[Figure, list[Axes]]:
        ...
        # Axes
        ax: list[Axes] = []
        ax.append(plt.subplot2grid(shape=(nrow, ncol), loc=(0, 0), colspan=3))
        ...
        sns.countplot(
            ...
            edgecolor="black",
            palette=sns.color_palette("blend:white,darkred", n_colors=len(counts_lipinsky)),
            ax=ax[-1],
        )
        ax[-1].set(xlabel="", ylabel="Count")
        ax[-1].set_title("Number of Lipinski's rule of five violations", loc="center")
        for container, lbl in zip(
            ax[-1].containers,
            [f"{100 * val / len(data_filtered):.0f}%" for val in counts_lipinsky.values],
        ):
            ax[-1].bar_label(container=container, labels=[lbl])
        ...
        def _plot_spider(data, ax, xticklabels, title, color):
            ...
            plt.xticks(ticks=angles[:-1], labels=xticklabels, size=9, position=(0.1, 0.1))
        ...
        # Plot
        colormap = plt.colormaps.get_cmap("Dark2").resampled(len(df))
    ...
    def hbond(self, figsize: tuple[float, float] = (7, 10)) -> tuple[Figure, list[Axes]]:
        ...
        fig, ax = plt.subplots(
            ...
            2,
            figsize=figsize,
            gridspec_kw={"width_ratios": [1.5, 1], "height_ratios": [1.5, 1, 1, 1, 1, 1, 1]},
        )
        ax = ax.flatten()
        ...
        def _bar_label(ax, full_data):
            ...
            ax.bar_label(
                ...
                padding=1,
                labels=[
                    f"{100 * val / len(full_data):.0f}%" for val in ax.containers[0].datavalues
                ],
            )
