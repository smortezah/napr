...
def find_best_models(
    ...
    Args:
        ...
        score: Score to use for hyperparameter optimization. Defaults to 'accuracy'.
        cv: Number of folds for cross-validation. Defaults to 5.
        directory: Directory to save the tuner. Defaults to '.' (current directory).
        project_name: Project name for the tuner. Defaults to 'tuner'.
        overwrite: Whether to overwrite the tuner if it already exists. Defaults to False.
    ...
    oracle = kt.oracles.BayesianOptimizationOracle(
        objective=kt.Objective("score", "max"), max_trials=max_trials
    )
    _cv = StratifiedKFold(cv, shuffle=True, random_state=random_state)
